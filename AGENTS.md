...
- OAuth client registrations persist to SQLite (`oauth_clients` table) — survive Railway deploys
- Refresh tokens (30-day) persist to SQLite (`oauth_tokens` table) — Claude Desktop can renew silently
- If MCP disconnects, most likely cause is an expired access token (7d) or a Railway deploy that severed the SSE connection
...
### execute_plan field naming — camelCase is now normalized
- `rome_execute_plan` operations accept both camelCase and snake_case field names
- `startDate`/`endDate` now map to `start_date`/`end_date` automatically before Zod validation
- Prior to this fix, camelCase dates in execute_plan were silently dropped (fields parsed as unknown, dates stayed null)
- **Always include dates explicitly** when creating/updating nodes — the normalization layer handles the rest
- The normalization happens in `services.ts` `normalizeNodeFields()` — add new aliases there if needed

### MCP OAuth — tokens and clients now persist to SQLite
- OAuth client registrations are now stored in `oauth_clients` table (survive Railway deploys)
- Access tokens last 7 days (up from 1 hour) — Claude Desktop won't expire mid-session
- Refresh tokens (30-day lifetime) are stored in `oauth_tokens` table — Claude can renew without re-auth
- Prior to this fix: every Railway deploy wiped in-memory client store, killing Claude Desktop's MCP connector
- **Auth codes (`_oauthCodes`) remain in-memory intentionally** — they expire in 5 minutes, persisting them to SQLite adds complexity for zero benefit. Only client registrations and refresh tokens need to survive deploys

### Graph isGoalNode — OBJ prefix only, not substring match
- `isGoalNode()` in `graphLayout.ts` previously matched any node whose name contained "goal" or "mission"
- This caused task nodes with those words in their names to be placed at graph center with a "GOAL" label
- Fixed: now requires `workstream === null` AND name matches `/^OBJ\d+:/i` — the explicit naming convention
- Nodes with null workstream (orphans) are now placed below center, not at (0,0) where they overlap the goal node
- **Never use loose substring matching for node-type detection** — use explicit prefix conventions or a DB field
...
- `oauth_clients` table: persisted MCP OAuth client registrations (survive deploys)
- `oauth_tokens` table: persisted MCP refresh tokens, 30-day expiry
